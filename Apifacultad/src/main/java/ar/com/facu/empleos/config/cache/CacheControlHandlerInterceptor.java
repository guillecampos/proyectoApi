package ar.com.facu.empleos.config.cache;
...
public class CacheControlHandlerInterceptor extends HandlerInterceptorAdapter implements HandlerInterceptor {
     ...
     * @param handler
     *            the handler for the given <code>request</code>
     */
    protected final void assignCacheControlHeader(final HttpServletRequest request, final HttpServletResponse response, final Object handler) {
	
	final CacheControl cacheControl = getCacheControl(request, response, handler);
	final String cacheControlHeader = createCacheControlHeader(cacheControl);
     ...
     *
     * @param cacheControl
     *            the <code>CacheControl</code> annotation from which to create the returned cache control header value
     ...
     *
     * @param cacheControl
     *            the <code>CacheControl</code> annotation from which to create the returned expires header value
     ...
     * @param response
     *            the current <code>HttpServletResponse</code>
     * @param handler
     ...
     */
    protected final CacheControl getCacheControl(final HttpServletRequest request, final HttpServletResponse response, final Object handler) {
	...
	assignCacheControlHeader(request, response, handler);
     ...
     *
     * @param useExpiresHeader
     *            <code>true</code> to set an expires header when a <code>CacheControl</code> annotation is present on a handler; <code>false</code> otherwise
     */
